...
abstract class TestCase extends  BaseTestCase
{
    ...
    {
        // Laravel registers a "storage/{path}" route for every filesystem
        // disk that has "serve" enabled. Disable it so route-counting
        // tests are deterministic across Laravel/Testbench versions.
        foreach (array_keys($app['config']->get('filesystems.disks', [])) as $disk) {
            $app['config']->set("filesystems.disks.{$disk}.serve", false);
        }
    }
